...
class ResponsiveBarChart extends StatelessWidget {
  const ResponsiveBarChart({super.key, required this.groups, required this.labels});
  ...
  Widget build(BuildContext context) => LayoutBuilder(builder: (context, constraints) {
    ...
    final style = Theme.of(context).textTheme.labelSmall!.copyWith(fontSize: 12);
    final values = [0.0, for (final group in groups) for (final rod in group.barRods) rod.toY];
    ...
    final fittedGroups = groups.map((group) {
      final width = (plotWidth / math.max(1, groups.length) * .65 /
          math.max(1, group.barRods.length)).clamp(1.0, 12.0);
      return group.copyWith(barsSpace: 1, barRods: [
        for (final rod in group.barRods) rod.copyWith(width: width),
      ]);
    }).toList();
    return Directionality(
      textDirection: TextDirection.ltr,
      child: SizedBox(height: 240 + 40 * scale, child: BarChart(BarChartData(
        minY: minY, maxY: maxY, barGroups: fittedGroups,
        titlesData: FlTitlesData(
          topTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
          rightTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
          leftTitles: AxisTitles(sideTitles: SideTitles(
            showTitles: true, reservedSize: axisWidth, interval: (maxY - minY) / 4,
            getTitlesWidget: (value, meta) => SideTitleWidget(meta: meta,
              child: Text(formatter.format(value), style: style)),
          )),
          bottomTitles: AxisTitles(sideTitles: SideTitles(
            showTitles: true, reservedSize: 32 * scale + 8, interval: 1,
            getTitlesWidget: (value, meta) => shown.contains(value.toInt())
              ? SideTitleWidget(meta: meta,
