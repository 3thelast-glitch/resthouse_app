...
class _UltimateDashboardPageState extends State<UltimateDashboardPage> {
  ...
  Future<void> _loadDashboardData() async {
    ...
    for (var b in bookings) {
      ...
      if (date.length >= 7) {
        ...
        mRevenue[month] = (mRevenue[month] ?? 0.0) + (b['total_price'] as num).toDouble();
      }
    }
    ...
    for (var e in expenses) {
      ...
      if (date.length >= 7) {
        final month = date.substring(0, 7); // yyyy-MM
        mExpenses[month] = (mExpenses[month] ?? 0.0) + (e['amount'] as num).toDouble();
      }
    }

    List<String> months = {...mRevenue.keys, ...mExpenses.keys}.toList()..sort();
    if (months.isEmpty) {
  ...
  void _showQuickAddRenter() {
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        title: const Text('إضافة مستأجر سريع', style: TextStyle(fontWeight: FontWeight.bold)),
        content: Form(
          key: formKey,
          child: Column(
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                ScaffoldMessenger.of(dialogContext).showSnackBar(const SnackBar(content: Text('تمت إضافة المستأجر بنجاح')));
              } catch (e) {
                if (!dialogContext.mounted) return;
  ...
  void _showQuickAddBooking() async {
    ...
    if (renters.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('الرجاء إضافة مستأجر أولاً قبل حجز الاستراحة')),
      );
      return;
    }
    ...
    showDialog(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setDialogState) => AlertDialog(
          title: const Text('تسجيل حجز سريع', style: TextStyle(fontWeight: FontWeight.bold)),
              ...
              children: [
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (value) {
                    String? notes;
                    if (value != null) {
                      final renter = renters.firstWhere((r) => r['phone'].toString() == value);
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (selectedPhone == null || startDate == null || endDate == null || price <= 0) {
                  ScaffoldMessenger.of(dialogContext).showSnackBar(const SnackBar(content: Text('الرجاء التحقق من المدخلات')));
                  return;
                }
                ...
                if (!dialogContext.mounted) return;
                Navigator.pop(dialogContext);
                ScaffoldMessenger.of(dialogContext).showSnackBar(const SnackBar(content: Text('تمت إضافة الحجز بنجاح')));
              },
              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0F766E), foregroundColor: Colors.white),
  ...
  void _showQuickAddExpense() {
    ...
    showDialog(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: const Text('تسجيل مصروف سريع', style: TextStyle(fontWeight: FontWeight.bold)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(controller: descController, decoration: const InputDecoration(labelText: 'وصف المصروف')),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (!dialogContext.mounted) return;
              Navigator.pop(dialogContext);
              ScaffoldMessenger.of(dialogContext).showSnackBar(const SnackBar(content: Text('تم تسجيل المصروف بنجاح')));
            },
            style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0F766E), foregroundColor: Colors.white),
                ...
                children: [
                  ...
                  Expanded(
                    child: _buildMetricCard(
                      ...
                      color: netProfit >= 0 ? const Color(0xFF0D9488) : const Color(0xFFDC2626),
                    ),
                  ),
                ],
              ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Wrap(
                              ...
                              runSpacing: 4,
                              children: [
                                _buildLegendIndicator(const Color(0xFF10B981), 'الإيرادات'),
                                _buildLegendIndicator(const Color(0xFFEF4444), 'المصروفات'),
                              ],
                            ),
                            const SizedBox(height: 24),
                            SizedBox(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(value, style: TextStyle(fontSize: 15.sp(context), fontWeight: FontWeight.bold, color: Colors.grey.shade800)),
                ],
              ),
            ),
          ],
  ...
  Widget _buildLegendIndicator(Color color, String text) {
    return Row(
      ...
      children: [
        ...
        Text(text, style: TextStyle(fontSize: 12.sp(context), color: Colors.grey, fontWeight: FontWeight.bold)),
      ],
    );
  }
      ...
      if (parts.length > 1) {
        return SideTitleWidget(
          ...
          child: Text(
            ...
            style: TextStyle(fontSize: 10.sp(context), fontWeight: FontWeight.bold, color: Colors.grey),
          ),
        );
      }
  ...
  Widget _buildActivitiesList() {
    if (_recentActivities.isEmpty) {
      return const SizedBox(
        height: 180,
        child: Center(
          child: Text('لا توجد أنشطة مسجلة بعد', style: TextStyle(color: Colors.grey)),
        ),
      );
    }
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return Row(
          children: [
            CircleAvatar(
              backgroundColor: isBooking ? const Color(0xFFECFDF5) : const Color(0xFFFEF2F2),
              foregroundColor: isBooking ? const Color(0xFF10B981) : const Color(0xFFEF4444),
              radius: 18,
              child: Icon(isBooking ? Icons.vpn_key_outlined : Icons.receipt_long, size: 16),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13.sp(context), color: const Color(0xFF1E293B)),
                  ),
            ...
            const SizedBox(width: 8),
            Text(
              isBooking ? '+${act.amount.toStringAsFixed(0)}' : '-${act.amount.toStringAsFixed(0)}',
              style: TextStyle(
                fontWeight: FontWeight.bold,
                fontSize: 13.sp(context),
                color: isBooking ? const Color(0xFF10B981) : const Color(0xFFEF4444),
              ),
            ),
          ],
        );
