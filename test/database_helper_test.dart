...
void main() {
  ...
  test(
    ...
    () async {
      ...
      await helper.insertRenter(renter('0555555555', 'مستأجر فعلي'));
      final realBookingId = await helper.insertBooking(
        booking(
          phone: '0555555555',
      ...
      final result = await helper.deleteDemoData();

      expect(result.deletedBookings, 7);
      expect(result.deletedRenters, 7);
      expect(result.deletedPayments, 0);
      expect(result.totalDeleted, 14);
      final remainingBookings = await helper.queryAllBookings();
      expect(remainingBookings, hasLength(1));
      expect(remainingBookings.single['id'], realBookingId);
      expect(await helper.queryAllRenters(), hasLength(1));
      expect(await helper.queryPaymentsForBooking(realBookingId), hasLength(1));
      expect(
        (await helper.queryRecentAuditEvents()).any(
          (event) => event['entity_type'] == 'demo_data',
        ),
        isTrue,
      );
    },
  ...
  test('يحافظ على الحجز التجريبي الذي استخدم ببيانات فعلية', () async {
    await helper.seedInitialData();
    final demoBooking = (await helper.queryAllBookings()).firstWhere(
      (booking) => (booking['total_price'] as num) > 0,
    );
    final demoBookingId = demoBooking['id'] as int;
    final demoPhone = demoBooking['phone'] as String;
    ...
    expect(result.deletedBookings, 6);
    expect(
      (await helper.queryAllBookings()).map((booking) => booking['id']),
      contains(demoBookingId),
    );
    expect(
      (await helper.queryAllRenters()).map((renter) => renter['phone']),
      contains(demoPhone),
    );
    expect(await helper.queryPaymentsForBooking(demoBookingId), hasLength(1));
  });

  test('يرقي البذور القديمة المطابقة فقط إلى سجلات تجريبية', () async {
    await helper.seedInitialData();
    await helper.insertRenter(renter('0555555555', 'مستأجر فعلي'));
    await helper.insertBooking(
      booking(
    ...
    await database.update(DatabaseHelper.tableBookings, {'is_demo': 0});
    await database.update(DatabaseHelper.tableRenters, {'is_demo': 0});
    await database.execute('PRAGMA user_version = 7');
    await helper.closeDatabase();

    final upgradedBookings = await helper.queryAllBookings();
    final upgradedRenters = await helper.queryAllRenters();
    expect(
      upgradedBookings
          .where((booking) => booking['phone'] == '0511111113')
          .single['is_demo'],
      1,
    );
    expect(
      upgradedRenters
          .where((renter) => renter['phone'] == '0511111113')
          .single['is_demo'],
      1,
    );
    expect(
      upgradedBookings
          .where((booking) => booking['phone'] == '0555555555')
          .single['is_demo'],
      0,
    );
    expect(
      upgradedRenters
          .where((renter) => renter['phone'] == '0555555555')
          .single['is_demo'],
      0,
    );
  });
